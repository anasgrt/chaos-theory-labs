...
docker cp "$HOME/labs/lab27/$1" lab27-control-plane:/etc/kubernetes/ce-lab27-apiserver.yaml || return
docker exec lab27-control-plane chmod 600 /etc/kubernetes/ce-lab27-apiserver.yaml || return
docker exec lab27-control-plane mv /etc/kubernetes/ce-lab27-apiserver.yaml /etc/kubernetes/manifests/kube-apiserver.yaml
}
...
for i in $(seq 1 90); do
  if k get --raw /readyz >/dev/null 2>&1 &&
     flags=$(k -n kube-system get pod kube-apiserver-lab27-control-plane -o json) &&
