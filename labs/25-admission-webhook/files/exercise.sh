...
for i in $(seq 1 30); do
  if endpoints=$(ksys get endpointslices -l kubernetes.io/service-name=admission -o json) &&
     printf '%s' "$endpoints" | jq -e '([.items[]?.endpoints[]?] | length) == 0' >/dev/null; then
    ksys get endpointslices -l kubernetes.io/service-name=admission -o yaml
    return
  fi
  sleep 1
done
