...
NODE = 'lab26-control-plane'
MARKER = b'ce-lab26-demo'
...
def read_record(name):
    ...
    result = subprocess.run(['docker', 'exec', NODE, 'crictl', 'exec', containers[0], 'etcdctl',
        ...
        'get', f'/registry/secrets/ce-lab26/{name}', '--write-out=json'],
        check=True, capture_output=True, text=True)
...
def main():
    ...
    target = Path.home() / 'labs/lab26' / f'{args.phase}-{args.name}.bin'
