...
install_provider() {
  ...
  docker cp "$HOME/labs/lab26/apiserver-$mode.json" lab26-control-plane:/etc/kubernetes/ce-lab26-next-apiserver.json || return
  docker exec lab26-control-plane chmod 600 /etc/kubernetes/ce-lab26-next-apiserver.json || return
  docker exec lab26-control-plane mv /etc/kubernetes/ce-lab26-next-apiserver.json /etc/kubernetes/manifests/kube-apiserver.yaml
}
...
wait_provider() {
  ...
  for i in {1..90}; do
    if k get --raw /livez >/dev/null 2>&1 &&
       state=$(k -n kube-system get pod kube-apiserver-lab26-control-plane -o json 2>/dev/null) &&
       printf '%s' "$state" | jq -e --arg mode "$mode" '
         ...
         any(.spec.containers[0].command[];
             . == ("--encryption-provider-config=/etc/kubernetes/ce-lab26/provider-" + $mode + ".json"))' >/dev/null; then
...
stored_secret() {
  python3 "$HOME/labs/lab26/storage_probe.py" "$@"
}
...
provider_summary() {
  ...
  jq '{resources: [.resources[] | {resources, providers:
     ...
     "$HOME/labs/lab26/provider-$mode.json"
}
