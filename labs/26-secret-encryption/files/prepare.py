...
def prepare(workspace):
    ...
    for name in ('key1', 'key2'):
        ...
        container['command'].append(f'--encryption-provider-config=/etc/kubernetes/ce-lab26/provider-{name}.json')
        container['volumeMounts'].append({'name': 'ce-lab26-encryption', 'mountPath': '/etc/kubernetes/ce-lab26', 'readOnly': True})
        manifest['spec']['volumes'].append({'name': 'ce-lab26-encryption', 'hostPath': {'path': '/etc/kubernetes/ce-lab26', 'type': 'Directory'}})
