...
| --- | --- | --- |
| [00](#lab-00) | Why does an isolated process view not reveal the sandbox's CPU budget? | docs/chaos-theory.md: §§5.2–5.5.2 and §5.7.1. |
| [01](#lab-01) | Why can one container prevent another from writing to a shared filesystem? | docs/chaos-theory.md: §5.4 and Experiment Card 5.1. The tmpfs fixture adapts the shared-storage example. |
| [02](#lab-02) | How does the same network delay affect one dependency exchange versus four sequential exchanges? | docs/chaos-theory.md: §§5.8–5.8.1 and Experiment Card 5.5. |
| [03](#lab-03) | What do ownership, disruption budgets and Service routing each guarantee during Pod replacement? | docs/chaos-theory.md: §§10.4, 10.5.1–10.5.2; Experiment Card 10.1. |
| [04](#lab-04) | Why can the same slow dependency pass one readiness probe and fail another? | docs/chaos-theory.md: §§10.4.4, 10.5.3–10.5.4; Experiment Card 10.2. |
| [05](#lab-05) | Can one startup SLI locate three different failure stages? | docs/chaos-theory.md: §§11.2, 11.4.1–11.4.3; scheduling in §12.1.1. |
| [06](#lab-06) | How does cordoning a node differ from losing its kubelet? | docs/chaos-theory.md: §§12.1.1–12.1.3 and 12.3.1–12.3.2. |
| [07](#lab-07) | Why can the same application shut down cleanly in one container and be killed in another? | docs/chaos-theory.md: §§5.13.1–5.13.2. |
| [08](#lab-08) | Is a failed container write caused by lost data, Unix permissions or a read-only mount? | docs/chaos-theory.md: §§5.13.3–5.13.4. |
| [09](#lab-09) | Did the workload fail before placement or after exceeding its memory limit? | docs/chaos-theory.md: §§10.6.1–10.6.2. |
| [10](#lab-10) | At which layer does a failed in-cluster HTTP request break? | docs/chaos-theory.md: §§10.7.1–10.7.2. |
| [11](#lab-11) | Why does a ConfigMap change not reach running Pods, and why can a rollout stall? | docs/chaos-theory.md: §§10.8.1–10.8.2. |
| [12](#lab-12) | Why can a container run out of processes while almost nothing is running? | docs/chaos-theory.md: §§5.14.1–5.14.2. |
| [13](#lab-13) | Which requests fail when one replica is deleted, and what actually repairs them? | docs/chaos-theory.md: §§10.9.1–10.9.2. |
| [14](#lab-14) | What does one name lookup cost inside a Pod, and what survives a DNS outage? | docs/chaos-theory.md: §§10.10.1–10.10.2. |
| [15](#lab-15) | What happens to a cluster when the webhook that approves new Pods stops answering? | docs/chaos-theory.md: §§12.4.1–12.4.2. |
| [16](#lab-16) | How much authority does this container actually have, and which flag granted it? | docs/chaos-theory.md: §§5.15.1–5.15.2. |
| [17](#lab-17) | Which benchmark controls does an untouched cluster fail, and what does remediating one actually cost? | docs/chaos-theory.md: §§12.6.1–12.6.2. |
| [18](#lab-18) | Which Pods does a namespace's security level actually reject, and when is that decision made? | docs/chaos-theory.md: §§12.5.1–12.5.2. |
| [19](#lab-19) | What can a workload do against the Kubernetes API, and which change grants or removes it? | docs/chaos-theory.md: §§10.11.1–10.11.2. |
| [20](#lab-20) | Where does a Secret's value actually rest, and who can read it? | docs/chaos-theory.md: §§12.7.1–12.7.2. |
| [21](#lab-21) | Which traffic does a NetworkPolicy stop, and what does the caller see when it does? | docs/chaos-theory.md: §§12.8.1–12.8.2. |
| [22](#lab-22) | Which ordinary-looking grant lets an account act as a stronger one? | docs/chaos-theory.md: §§12.9.1–12.9.2. |
| [23](#lab-23) | When should a probe withdraw traffic, restart a container, or give it more time to start? | docs/chaos-theory.md: §§10.12.1, 10.12.2 (maintained supplement; primary references linked there). |
| [24](#lab-24) | Why can a claim wait, a replacement Pod keep data, and a retained volume still refuse a new claim? | docs/chaos-theory.md: §§10.13.1, 10.13.2 (maintained supplement; primary references linked there). |
| [25](#lab-25) | Why can an idle namespace reject a Pod, and why does lowering its quota leave existing Pods running? | docs/chaos-theory.md: §§12.10.1, 12.10.2 (maintained supplement; primary references linked there). |
| [26](#lab-26) | How can the API return the same Secret while storage changes, and why can a live API lose access to it? | docs/chaos-theory.md: §§12.11.1, 12.11.2 (maintained supplement; primary references linked there). |

<a id="lab-00"></a>

## Lab 00 — A container is several Linux mechanisms
...
ls ~/labs/lab00/rootfs
systemctl is-active ce-lab00.scope; echo "scope status=$? (nonzero means not active)"
```

**Record:** Preconditions: the prepared root's directory names and the ce-lab00 scope state.
...
```bash
sudo systemd-run --unit=ce-lab00 --scope --collect \
  -p MemoryMax=128M -p MemorySwapMax=0 -p TasksMax=50 -p CPUQuota=20% \
  unshare --fork --pid --mount --mount-proc="$HOME/labs/lab00/rootfs/proc" \
  chroot "$HOME/labs/lab00/rootfs" /bin/sh
```
...
```bash
systemctl is-active ce-lab00.scope; echo "status=$? (nonzero means not active)"
```

**Record:** The final ce-lab00 scope state.
...
```bash
sudo systemctl stop ce-lab00.scope
sudo systemctl reset-failed ce-lab00.scope 2>/dev/null || true
```
...
## Lab 01 — Separate containers can share a full filesystem
...
```bash
mkdir -p ~/labs/lab01/shared
if mountpoint -q "$HOME/labs/lab01/shared"; then
  echo 'STOP: a previous mount remains. Complete recovery or reset before restarting.'
else
  sudo mount -t tmpfs -o size=32m tmpfs "$HOME/labs/lab01/shared"
fi
findmnt --mountpoint "$HOME/labs/lab01/shared" -o TARGET,FSTYPE,SIZE,AVAIL
```
...
```bash
docker run --rm --name ce-lab01-writer -v "$HOME/labs/lab01/shared:/data" ubuntu:24.04 sh -c 'dd if=/dev/zero of=/data/probe bs=1M count=1'
echo "baseline probe exit=$?"
df -h ~/labs/lab01/shared
sudo rm -f ~/labs/lab01/shared/probe
```
...
```bash
if [ "$(findmnt --mountpoint "$HOME/labs/lab01/shared" --noheadings --output FSTYPE)" = tmpfs ]; then
  docker run --rm --name ce-lab01-writer -v "$HOME/labs/lab01/shared:/data" ubuntu:24.04 sh -c 'dd if=/dev/zero of=/data/full bs=1M count=40; echo "filling dd exit=$?"; df -h /data'
else
...
```bash
sudo rm -f ~/labs/lab01/shared/full ~/labs/lab01/shared/probe
docker run --rm --name ce-lab01-writer -v "$HOME/labs/lab01/shared:/data" ubuntu:24.04 sh -c 'dd if=/dev/zero of=/data/probe bs=1M count=1'
...
```bash
sudo rm -f ~/labs/lab01/shared/probe
sudo umount ~/labs/lab01/shared
mountpoint ~/labs/lab01/shared; echo "status=$? (nonzero means unmounted)"
docker ps -a --filter 'name=^/ce-lab01-'
```

**Record:** Cleanup: the mountpoint result and the container listing.

**Recovery check:** No ce-lab01 test container remains and the shared tmpfs is unmounted.
...
**Expected:** The directory is no longer a mountpoint and no ce-lab01 container remains.
...
## Lab 02 — Network delay accumulates across sequential calls
...
- A Python echo server runs in ce-lab02 on port 8070. The host client measures one or four exchanges per sample. Change only the container’s eth0 egress: no delay → 1 ms control → 25 ms → no delay.
...
```bash
PID=$(docker inspect -f '{{.State.Pid}}' ce-lab02)
if [ "$PID" -gt 1 ]; then
...
netem delay 1ms and then 25ms should appear only on ce-lab02's eth0. Increasing Sent packet/byte counters between the before and after snapshots establish that measured traffic crossed the queue; configuration alone does not.
...
## Lab 03 — Ownership, disruption budgets and Service routing
...
- **k helper:** Runs kubectl against kind-lab03 in namespace chaos-labs. Node queries are cluster-scoped even though the helper specifies a namespace.
...
```bash
k create pdb lab03-budget --selector=app=goldpinger --min-available=3 --dry-run=client -o yaml | k apply -f -
k wait pdb/lab03-budget --for=jsonpath='{.status.currentHealthy}'=3 --timeout=60s
k wait pdb/lab03-budget --for=jsonpath='{.status.disruptionsAllowed}'=0 --timeout=60s
k get pdb lab03-budget
printf '{"apiVersion":"policy/v1","kind":"Eviction","metadata":{"name":"%s","namespace":"chaos-labs"}}\n' "$victim" > ~/labs/lab03/eviction.json
k create --raw "/api/v1/namespaces/chaos-labs/pods/$victim/eviction" -f ~/labs/lab03/eviction.json
...
timeout 60s kubectl --kubeconfig="$HOME/labs/lab03/kubeconfig" --context=kind-lab03 --namespace=chaos-labs get pods -l app=goldpinger -w
...
```bash
k delete pdb lab03-budget --ignore-not-found
k patch svc goldpinger --type merge -p '{"spec":{"selector":{"experiment":"lab03-no-match"}}}'
sleep 5
...
**Recovery check:** Three replicas and three eligible Service endpoints are restored, HTTP succeeds, and lab03-budget is absent.
...
## Lab 04 — Health depends on what you test
...
source ~/labs/lab04/connect-proxy.sh
...
source ~/labs/lab04/connect-proxy.sh ~/labs/lab04/slow-http.yaml
curl -fsS --max-time 5 "$T/proxies/slow/toxics"; echo
pings
readiness
```
...
## Lab 05 — Locate startup failures and measure recovery
...
```bash
source ~/labs/lab05/env.sh
k delete -f ~/labs/lab05/startup.yaml --ignore-not-found --grace-period=1
```
...
## Lab 06 — Placement, supervision and replacement
...
The node-web workload prefers lab06-worker but does not require it. A preference can be overridden when that node is unavailable; a required hostname constraint could instead leave a replacement Pending. Record placement rather than assuming that replica count guarantees independent failure domains. All kind nodes still share this VM.
...
```bash
source ~/labs/lab06/exercise.sh
source ~/labs/lab06/env.sh
...
kubectl --kubeconfig="$HOME/labs/lab06/kubeconfig" -n kube-node-lease get lease "$NODE" -o jsonpath='{.spec.renewTime}{"\n"}'
```
...
STOP_EPOCH=$(date +%s)
printf '%s\n' "$STOP_EPOCH" > ~/labs/lab06/stopped-at.txt
date --iso-8601=seconds
...
```bash
for node in lab06-worker lab06-worker2 lab06-worker3; do
  docker exec "$node" systemctl start kubelet
done

# After the API responds, restore placement eligibility too.
source ~/labs/lab06/env.sh
k uncordon lab06-worker lab06-worker2 lab06-worker3
```
...
## Lab 07 — Signal delivery and the shutdown budget
...
docker create --name ce-lab07-wrapper --stop-signal SIGTERM python:3.12-slim sh -c 'trap "" TERM; python -u /worker.py & wait'
start_worker ce-lab07-wrapper
```
...
```bash
time docker stop --timeout 5 ce-lab07-wrapper
docker logs ce-lab07-wrapper
docker inspect -f '{{json .State}}' ce-lab07-wrapper
docker cp ce-lab07-wrapper:/cleanup-complete ~/labs/lab07/wrapper-complete.txt
```
...
```bash
docker create --name ce-lab07-exec --stop-signal SIGTERM python:3.12-slim sh -c 'exec python -u /worker.py'
start_worker ce-lab07-exec
```
...
```bash
time docker stop --timeout 5 ce-lab07-exec
docker logs ce-lab07-exec
docker inspect -f '{{json .State}}' ce-lab07-exec
docker cp ce-lab07-exec:/cleanup-complete ~/labs/lab07/exec-complete.txt
cat ~/labs/lab07/exec-complete.txt
```
...
```bash
docker create --name ce-lab07-short --stop-signal SIGTERM python:3.12-slim sh -c 'exec python -u /worker.py'
start_worker ce-lab07-short
```
...
```bash
time docker stop --timeout 1 ce-lab07-short
docker logs ce-lab07-short
docker inspect -f '{{json .State}}' ce-lab07-short
docker cp ce-lab07-short:/cleanup-complete ~/labs/lab07/short-complete.txt
```
...
```bash
docker rm ce-lab07-wrapper ce-lab07-exec ce-lab07-short
docker ps -a --filter 'name=^/ce-lab07-' --format '{{.Names}}'
```

**Record:** Whether any ce-lab07 container remains, once all three rows are complete.
...
**Expected:** No ce-lab07 containers remain; the successfully copied exec-complete.txt file remains in the VM workspace.
...
## Lab 08 — Persistence, ownership and read-only mounts
...
- Use only the named volume ce-lab08-data and disposable ce-lab08 containers. Setup resets this exercise's data; save observations elsewhere.
...
```bash
docker run --name ce-lab08-original --mount type=volume,src=ce-lab08-data,dst=/data python:3.12-slim sh -c 'echo layer > /ephemeral.txt; echo volume > /data/persisted.txt; cat /ephemeral.txt /data/persisted.txt; ls -ldn /data'
docker cp ce-lab08-original:/ephemeral.txt ~/labs/lab08/original-layer.txt
docker inspect -f '{{json .Mounts}}' ce-lab08-original
```
...
```bash
docker rm ce-lab08-original
docker run --rm --name ce-lab08-check --mount type=volume,src=ce-lab08-data,dst=/data python:3.12-slim sh -c 'cat /data/persisted.txt; test ! -e /ephemeral.txt && echo writable-layer-file-absent; ls -ldn /data'
```
...
```bash
docker run --rm --name ce-lab08-check --user 10001:10001 --mount type=volume,src=ce-lab08-data,dst=/data python:3.12-slim sh -c 'id; ls -ldn /data; echo attempt > /data/user.txt'
echo "write exit=$?"
```
...
```bash
docker run --rm --name ce-lab08-check --mount type=volume,src=ce-lab08-data,dst=/data python:3.12-slim chown 10001:10001 /data
docker run --rm --name ce-lab08-check --user 10001:10001 --mount type=volume,src=ce-lab08-data,dst=/data python:3.12-slim sh -c 'id; ls -ldn /data; echo attempt > /data/user.txt'
echo "write exit=$?"
```
...
```bash
docker run --name ce-lab08-readonly --user 10001:10001 --mount type=volume,src=ce-lab08-data,dst=/data,readonly python:3.12-slim sh -c 'id; ls -ldn /data; echo attempt > /data/user.txt'
echo "write exit=$?"
docker inspect -f '{{json .Mounts}}' ce-lab08-readonly
```
...
```bash
docker run --name ce-lab08-recovered --user 10001:10001 --mount type=volume,src=ce-lab08-data,dst=/data python:3.12-slim sh -c 'ls -ldn /data; echo recovered > /data/user.txt && cat /data/user.txt /data/persisted.txt'
echo "write/read exit=$?"
docker inspect -f '{{json .Mounts}}' ce-lab08-recovered
```
...
```bash
docker rm ce-lab08-readonly ce-lab08-recovered
docker volume rm ce-lab08-data
docker ps -a --filter 'name=^/ce-lab08-' --format '{{.Names}}'
docker volume ls --filter name=ce-lab08-data
```

**Record:** Whether any lab container or the ce-lab08-data volume remains, once all six rows are filled.
...
**Expected:** Both files exist in the original container; only /data is backed by ce-lab08-data.
...
## Lab 09 — Unscheduled Pods versus memory-killed containers
...
- Use namespace ce-lab09 on this lab’s chaos cluster. Compare a healthy HTTP process, an impossible CPU request and a bounded memory allocation; then restore the healthy fixture.
...
- **k:** kubectl using Lab 09’s private kubeconfig, kind-lab09 context and only namespace ce-lab09.
...
k apply -f ~/labs/lab09/unscheduled.yaml
...
```bash
k get pod resource-web -o json > ~/labs/lab09/oom-evidence.json
k describe pod resource-web
k get pod resource-web -o custom-columns=NAME:.metadata.name,UID:.metadata.uid
k logs resource-web --previous | tee ~/labs/lab09/previous.log
uid=$(k get pod resource-web -o jsonpath='{.metadata.uid}')
...
```bash
k delete pod resource-web --wait=true --timeout=30s
k apply -f ~/labs/lab09/good.yaml
...
## Lab 10 — Trace DNS, Service ports and the application listener
...
An ordinary ClusterIP Service has a DNS name scoped by namespace. web.ce-lab10.svc.cluster.local identifies this fixture in kind's cluster.local domain. A wrong namespace in the name can fail resolution while the correct Service IP still works. Diagnose that difference before changing the cluster DNS server.
...
- Use namespace ce-lab10 with one Python server and one Python client. Change the requested name, then targetPort, then the bind address; restore each fault before the next comparison.
...
- **k:** kubectl fixed to kind-lab10 and ce-lab10. The client remains unchanged while the server is intentionally recreated for the bind comparison.
...
resolve web.ce-lab10.svc.cluster.local
probe http://web.ce-lab10.svc.cluster.local/
probe "http://$SVC_IP/"
...
```bash
resolve web.ce-lab10-missing.svc.cluster.local
echo "wrong-name DNS exit=$?"
probe http://web.ce-lab10-missing.svc.cluster.local/
...
k apply -f ~/labs/lab10/loopback.yaml
...
```bash
k delete pod web --wait=true --timeout=30s
k apply -f ~/labs/lab10/server.yaml
...
**Expected:** Sourcing the helpers targets only Lab 10. The port check succeeds only when a nonempty current slice list uses the requested port.
...
## Lab 11 — Configuration freshness and a stalled rollout
...
- Use namespace ce-lab11, two Python HTTP replicas, an environment-backed message and a deliberately wrong readiness port. Query every Pod directly through the API proxy; this measures application responses, not Service routing.
...
- **k / responses:** kubectl targets kind-lab11 and ce-lab11. responses prints each Pod name and its HTTP body; a missing or failed query returns nonzero.
...
```bash
stable_revision=$(cat ~/labs/lab11/stable-revision.txt)
...
## Lab 12 — Orphan reaping and the container process budget
...
```bash
source ~/labs/lab12/exercise.sh
docker create --name ce-lab12-noinit --pids-limit 64 python:3.12-slim python -u -c 'import os, time; print(f"application pid={os.getpid()} reaping=no", flush=True); time.sleep(3600)'
start_process_container ce-lab12-noinit
docker exec ce-lab12-noinit python -u /procreport.py
```
...
```bash
docker exec ce-lab12-noinit python -u /orphan-maker.py 40
sleep 2
docker exec ce-lab12-noinit python -u /procreport.py
docker logs ce-lab12-noinit
```
...
```bash
docker exec ce-lab12-noinit python -u /fork-burst.py 40
docker exec ce-lab12-noinit python -u /procreport.py
```
...
```bash
docker create --name ce-lab12-init --pids-limit 64 python:3.12-slim python -u /reaper.py
start_process_container ce-lab12-init
docker exec ce-lab12-init python -u /procreport.py
docker exec ce-lab12-init python -u /orphan-maker.py 40
sleep 2
docker exec ce-lab12-init python -u /procreport.py
docker logs ce-lab12-init --tail 5
```
...
```bash
docker exec ce-lab12-init python -u /fork-burst.py 40
docker exec ce-lab12-init python -u /procreport.py
```
...
```bash
docker exec ce-lab12-init python -u /fork-burst.py 200
docker exec ce-lab12-init python -u /procreport.py
nproc
...
```bash
docker rm -f ce-lab12-noinit ce-lab12-init
docker ps -a --filter 'name=^/ce-lab12-' --format '{{.Names}}'
ps -eo pid --no-headers | wc -l
```

**Record:** Whether any ce-lab12 container remains, and the final host process count.
...
The host can see the container processes, so its count can change. The 64-task ceiling applies to this container’s cgroup; it does not impose a 64-task limit on the VM. After the bursts reap their children, pids.current should fall back toward the baseline. Removing both containers should leave no ce-lab12 containers. The limit bounded the blast radius of a fork storm; it did not reap zombies or reduce demand.
...
**Expected:** No ce-lab12 containers should remain, and the host count should be close to the value recorded before the bursts.
...
## Lab 13 — Endpoint removal, preStop and the grace budget
...
- Use namespace ce-lab13 with two replicas and one client. Delete one replica under a steady 25 requests per second four times, changing only the shutdown behaviour, then only the hook, then only the budget.
...
- **k / drain_case:** kubectl fixed to kind-lab13 and ce-lab13. drain_case NAME runs one whole case: settle, probe, delete at t=4s, sample endpoints, then print the timeline, probe result and server log.
...
```bash
k apply -f ~/labs/lab13/web-abrupt.yaml
...
```bash
k apply -f ~/labs/lab13/web-prestop.yaml
k rollout status deployment/drain-web --request-timeout=0 --timeout=120s
shutdown_settings
drain_case prestop
```
...
```bash
k apply -f ~/labs/lab13/web-shortgrace.yaml
k rollout status deployment/drain-web --request-timeout=0 --timeout=120s
shutdown_settings
...
## Lab 14 — Search paths, ndots and a resolver outage

**Question:** What does one name lookup cost inside a Pod, and what survives a DNS outage?

**Before you run:** Provision the shared VM once with ./lab.sh provision. Run ./lab.sh 14 setup on the host. Outbound DNS from the VM is required for the external-name comparison. No other lab is required.
...
- Use namespace ce-lab14 with one Service and two clients that differ only in ndots. Count CoreDNS queries for in-cluster, external, absolute and absent names, then remove the resolver and compare paths.
...
- **k / ksys / dns_count:** kubectl fixed to kind-lab14, for ce-lab14 and for kube-system. dns_count POD NAME... prints the measured cost of each lookup.
...
```bash
dns_count client web web.ce-lab14.svc.cluster.local.
...
```bash
dns_count client chaos-lab14-absent.example
dns_count client-ndots chaos-lab14-absent.example
```
...
printf '%s\n' "$original" | tee ~/labs/lab14/coredns-replicas.txt
...
```bash
ksys scale deployment coredns --replicas="$(cat ~/labs/lab14/coredns-replicas.txt)"
...
## Lab 15 — Admission control as a synchronous dependency
...
Scope decides the blast radius. This webhook selects only namespaces labelled ce-lab15=scoped, with a rule limited to CREATE on Pods, so unlabelled namespaces are never consulted and deletions, Deployment writes and other operations are never intercepted.
...
The API server calls the webhook over HTTPS and verifies its certificate against the caBundle in the configuration. Setup generates one self-signed certificate for admission.ce-lab15-system.svc and installs it in both places, so a failure here comes from the injected fault rather than from trust. Server-side dry run also invokes the webhook, which tests enforcement without creating anything.
...
- Use the scoped namespace ce-lab15 and the unscoped backend namespace ce-lab15-system. Remove the webhook backend, observe direct creation, controller-driven creation and an unscoped namespace, then compare failurePolicy values before recovering.
- wait_backend_removed waits for an empty backend set; wait_backend_ready waits for a ready endpoint. Both print EndpointSlice evidence and return an error if the expected state is not observed within 30 checks.
...
- **k / ksys:** kubectl fixed to kind-lab15 for the scoped namespace ce-lab15 and for the unscoped backend namespace ce-lab15-system.
...
source ~/labs/lab15/helpers.sh
webhook_state
backend_state
k apply -f ~/labs/lab15/labelled-pod.yaml
k wait pod/labelled --for=condition=Ready --timeout=60s --request-timeout=0
k apply -f ~/labs/lab15/unlabelled-pod.yaml
...
wait_backend_removed
backend_state
time k apply -f ~/labs/lab15/labelled-pod.yaml
...
```bash
k apply -f ~/labs/lab15/workload.yaml
...
ksys apply -f ~/labs/lab15/system-pod.yaml
echo "unscoped create exit=$?"
ksys get pod unscoped -o custom-columns='NAME:.metadata.name,PHASE:.status.phase'
ksys get namespace ce-lab15 ce-lab15-system -o custom-columns='NAME:.metadata.name,SCOPED:.metadata.labels.ce-lab15'
```
...
```bash
k patch validatingwebhookconfiguration ce-lab15-pods --type=json -p '[{"op":"replace","path":"/webhooks/0/failurePolicy","value":"Ignore"}]'
webhook_state
k apply -f ~/labs/lab15/unlabelled-pod.yaml
echo "unlabelled create exit=$?"
sleep 5
...
wait_backend_ready
k patch validatingwebhookconfiguration ce-lab15-pods --type=json -p '[{"op":"replace","path":"/webhooks/0/failurePolicy","value":"Fail"}]'
for i in $(seq 1 30); do
  k apply -f ~/labs/lab15/labelled-pod.yaml --dry-run=server >/dev/null 2>&1 && break
  sleep 2
done
webhook_state
backend_state
...
k apply -f ~/labs/lab15/labelled-pod.yaml
k wait pod/labelled --for=condition=Ready --timeout=60s --request-timeout=0
k apply -f ~/labs/lab15/unlabelled-pod.yaml --dry-run=server
echo "dry-run exit=$?"
ksys logs deployment/admission --tail=10
```
...
The baseline should show failurePolicy Fail, a five-second timeout, the ce-lab15=scoped selector and one ready backend endpoint. The labelled Pod should be created and the unlabelled Pod rejected with the policy message, with one review line logged for each. A rejection quoting the policy is the check working; a rejection naming the webhook call itself would mean the dependency failed.
...
## Lab 16 — Container privilege boundaries and hardening
...
```bash
docker run --name ce-lab16-default -v ~/labs/lab16/report.py:/report.py:ro python:3.12-slim python -u /report.py
docker inspect -f 'User={{.Config.User}} Privileged={{.HostConfig.Privileged}} PidMode={{.HostConfig.PidMode}} ReadonlyRootfs={{.HostConfig.ReadonlyRootfs}} CapDrop={{.HostConfig.CapDrop}}' ce-lab16-default
```
...
```bash
docker run --name ce-lab16-hostpid --pid=host -v ~/labs/lab16/report.py:/report.py:ro python:3.12-slim python -u /report.py
docker inspect -f 'PidMode={{.HostConfig.PidMode}} Privileged={{.HostConfig.Privileged}}' ce-lab16-hostpid
```
...
```bash
docker run --name ce-lab16-privileged --privileged -v ~/labs/lab16/report.py:/report.py:ro python:3.12-slim python -u /report.py
docker inspect -f 'Privileged={{.HostConfig.Privileged}} SecurityOpt={{.HostConfig.SecurityOpt}}' ce-lab16-privileged
```
...
```bash
docker run --name ce-lab16-hardened --user 10001:10001 --cap-drop ALL --security-opt no-new-privileges --read-only --tmpfs /tmp -v ~/labs/lab16/report.py:/report.py:ro python:3.12-slim python -u /report.py
docker inspect -f 'User={{.Config.User}} CapDrop={{.HostConfig.CapDrop}} ReadonlyRootfs={{.HostConfig.ReadonlyRootfs}} SecurityOpt={{.HostConfig.SecurityOpt}} Tmpfs={{.HostConfig.Tmpfs}}' ce-lab16-hardened
```
...
for name in default hostpid privileged hardened; do
  printf '== %s\n' "$name"
  docker logs "ce-lab16-$name"
done
docker rm -f ce-lab16-default ce-lab16-hostpid ce-lab16-privileged ce-lab16-hardened
docker ps -a --filter 'name=^/ce-lab16-' --format '{{.Names}}'
```

**Record:** The four cases side by side, and whether any ce-lab16 container remains.
...
**Expected:** The four rows should differ only where a flag changed a boundary, and no ce-lab16 container should remain afterwards.
...
## Lab 17 — Reading a cluster against the CIS benchmark
...
- Scan the lab17 cluster, then change one file mode, one API server flag and one RBAC binding, re-scanning after each change and after restoring it.
...
- **k / cis_scan:** kubectl fixed to kind-lab17, and the scan helper that runs the six checks against this cluster.
...
source ~/labs/lab17/helpers.sh
cis_scan | tee ~/labs/lab17/baseline.txt
```
...
```bash
docker exec lab17-control-plane chmod 644 /etc/kubernetes/admin.conf
docker exec lab17-control-plane stat -c '%n %a %U:%G' /etc/kubernetes/admin.conf
cis_scan
```
...
```bash
docker exec lab17-control-plane chmod 600 /etc/kubernetes/admin.conf
cis_scan
```
...
diff -u ~/labs/lab17/kube-apiserver-original.yaml ~/labs/lab17/kube-apiserver-profiling.yaml
...
k create clusterrolebinding ce-lab17-audit --clusterrole=cluster-admin --serviceaccount=ce-lab17:audit-demo
cis_scan
k delete clusterrolebinding ce-lab17-audit
k delete serviceaccount audit-demo
cis_scan | tee ~/labs/lab17/final.txt
diff ~/labs/lab17/baseline.txt ~/labs/lab17/final.txt && echo 'final scan matches the baseline'
```
...
source ~/labs/lab17/exercise.sh
install_apiserver kube-apiserver-original.yaml
docker exec lab17-control-plane crictl ps --name kube-apiserver
wait_profiling default
k get nodes
```
...
## Lab 18 — Pod Security admission and what a namespace label blocks
...
- Use namespace ce-lab18 with three Pod manifests. Move the same namespace from unlabelled to baseline, to restricted, to warn and back, recording which creations succeed and what each message says.
...
- **namespace labels:** The enforce, warn and audit keys currently set on ce-lab18. Removing a key removes that mode; nothing is inherited from the cluster.
...
- **k:** kubectl fixed to kind-lab18 and ce-lab18. The namespace labels are changed with kubectl label, and removed with a trailing hyphen.
...
```bash
source ~/labs/lab18/exercise.sh
source ~/labs/lab18/helpers.sh
k get namespace ce-lab18 -o jsonpath='{.metadata.labels}{"\n"}'
k apply -f ~/labs/lab18/privileged.yaml
...
```bash
k label namespace ce-lab18 pod-security.kubernetes.io/enforce=baseline --overwrite
k get namespace ce-lab18 -o jsonpath='{.metadata.labels}{"\n"}'
...
```bash
k label namespace ce-lab18 pod-security.kubernetes.io/enforce=restricted --overwrite
k apply -f ~/labs/lab18/plain.yaml
echo "plain create exit=$?"
k apply -f ~/labs/lab18/hardened.yaml
...
```bash
k label namespace ce-lab18 pod-security.kubernetes.io/enforce-
k label namespace ce-lab18 pod-security.kubernetes.io/warn=restricted --overwrite
k get namespace ce-lab18 -o jsonpath='{.metadata.labels}{"\n"}'
k apply -f ~/labs/lab18/plain.yaml
echo "plain create exit=$?"
...
k label namespace ce-lab18 pod-security.kubernetes.io/warn-
k label namespace ce-lab18 pod-security.kubernetes.io/enforce=restricted --overwrite
k get namespace ce-lab18 -o jsonpath='{.metadata.labels}{"\n"}'
k apply -f ~/labs/lab18/plain.yaml --dry-run=server
echo "dry-run exit=$?"
k get pods -o custom-columns='NAME:.metadata.name,PHASE:.status.phase'
...
**Recovery check:** Restricted enforcement was demonstrated with a rejected dry run, then the hardened Pod and the namespace label were removed, returning ce-lab18 to the unlabelled state the lab started from.
...
## Lab 19 — What a Pod's ServiceAccount can reach
...
- Use namespace ce-lab19 with two identical clients that differ only in whether the token is mounted. Compare the default account, an explicit Role and RoleBinding, another namespace, and the removal of both the grant and the token.
...
- **k:** kubectl fixed to kind-lab19 and ce-lab19. The probe runs inside a Pod and uses only what that Pod is given.
...
k auth can-i list pods --as=system:serviceaccount:ce-lab19:default --namespace=ce-lab19
echo "can-i exit=$?"
```
...
```bash
k apply -f ~/labs/lab19/rbac.yaml
k get role,rolebinding pod-reader -o custom-columns='KIND:.kind,NAME:.metadata.name'
k exec client -- python -u /scripts/apicheck.py
k auth can-i list pods --as=system:serviceaccount:ce-lab19:default --namespace=ce-lab19
k auth can-i list pods --as=system:serviceaccount:ce-lab19:default --namespace=kube-system
...
## Lab 20 — What a Secret protects, and what it does not
...
- Use namespace ce-lab20 with one Secret and two consumers that differ only in how they receive it, then read the same value through the API, through etcd, through both containers, and after a rotation.
...
k -n kube-system get pod kube-apiserver-lab20-control-plane -o jsonpath='{.spec.containers[0].command}{"\n"}'
```
...
```bash
etcd_get /registry/secrets/ce-lab20/app-credentials > ~/labs/lab20/stored.bin
echo "etcd read exit=$?"
ls -l ~/labs/lab20/stored.bin
strings ~/labs/lab20/stored.bin
```
...
k auth can-i get secrets --as=system:serviceaccount:ce-lab20:no-secret-access --namespace=ce-lab20
echo "can-i exit=$?"
```
...
```bash
k create secret generic app-credentials --from-literal=password=rotated-lab20 \
  --dry-run=client -o yaml | k apply -f -
k get secret app-credentials -o jsonpath='{.data.password}' | base64 -d; echo
wait_secret rotated-lab20
k exec file-consumer -- python -u /scripts/secretprobe.py
k exec env-consumer -- python -u /scripts/secretprobe.py
```
...
```bash
k create secret generic app-credentials --from-literal=password=lab20-original \
  --dry-run=client -o yaml | k apply -f -
wait_secret lab20-original
k exec file-consumer -- python -u /scripts/secretprobe.py
...
```bash
kubectl --kubeconfig="$HOME/labs/lab20/kubeconfig" --context=kind-lab20 --namespace=ce-lab20 \
  create secret generic app-credentials --from-literal=password=lab20-original --dry-run=client -o yaml |
  kubectl --kubeconfig="$HOME/labs/lab20/kubeconfig" --context=kind-lab20 --namespace=ce-lab20 apply -f -
kubectl --kubeconfig="$HOME/labs/lab20/kubeconfig" --context=kind-lab20 --namespace=ce-lab20 get pods
```
...
The API returns .data.password as base64 and one decode prints the password; kubectl describe shows only the key and its size. etcd holds the same password as readable bytes beside the key /registry/secrets/ce-lab20/app-credentials, because no encryption-provider-config is configured on this API server. Encoding and storage are separate from access control, and only the second is changed by encryption at rest.
...
## Lab 21 — Default deny, and what a policy does not stop
...
- Use namespace ce-lab21 with one server behind a Service and two identical clients that differ only in a label, then add a default deny, an allow rule, an egress deny and a DNS allow, one at a time.
...
```bash
k apply -f ~/labs/lab21/deny-ingress.yaml
...
```bash
k apply -f ~/labs/lab21/allow-client.yaml
...
```bash
k apply -f ~/labs/lab21/deny-egress.yaml
sleep 5
probe client dns server
...
```bash
k apply -f ~/labs/lab21/allow-dns-egress.yaml
...
```bash
k delete -f ~/labs/lab21/deny-ingress.yaml -f ~/labs/lab21/allow-client.yaml -f ~/labs/lab21/allow-dns-egress.yaml --ignore-not-found
k get networkpolicy
...
```bash
kubectl --kubeconfig="$HOME/labs/lab21/kubeconfig" --context=kind-lab21 --namespace=ce-lab21 \
  delete networkpolicy --all --ignore-not-found
kubectl --kubeconfig="$HOME/labs/lab21/kubeconfig" --context=kind-lab21 --namespace=ce-lab21 get pods
```
...
## Lab 22 — Grants that reach further than they name
...
- Use namespace ce-lab22 with three ServiceAccounts whose Roles do not overlap, then read Secrets from the stronger account, try to widen a Role, and compare four ways of checking exec access.
...
```bash
source ~/labs/lab22/helpers.sh
k get role -o custom-columns='NAME:.metadata.name,RULES:.rules[*].resources'
k auth can-i create pods --as=system:serviceaccount:ce-lab22:builder
k auth can-i get secrets --as=system:serviceaccount:ce-lab22:builder
...
```bash
k --as=system:serviceaccount:ce-lab22:builder apply -f ~/labs/lab22/borrowed-pod.yaml
echo "create exit=$?"
...
```bash
k auth can-i get secrets --as=system:serviceaccount:ce-lab22:role-writer
k --as=system:serviceaccount:ce-lab22:role-writer create role within-rights --verb=get --resource=roles
echo "within rights exit=$?"
k --as=system:serviceaccount:ce-lab22:role-writer create role beyond-rights --verb=get --resource=secrets
...
```bash
k auth can-i create pods/exec --as=system:serviceaccount:ce-lab22:builder
k auth can-i create pods --subresource=exec --as=system:serviceaccount:ce-lab22:builder
cat ~/labs/lab22/exec-review.yaml
k create -f ~/labs/lab22/exec-review.yaml -o jsonpath='{.status.allowed}{"\n"}'
k --as=system:serviceaccount:ce-lab22:builder exec borrowed-pod -- echo reached-the-container
echo "exec exit=$?"
```
...
k exec builder-pod -- python -u /scripts/apiprobe.py
k auth can-i get secrets --as=system:serviceaccount:ce-lab22:secret-reader
echo "reader secrets can-i exit=$?"
k apply -f ~/labs/lab22/rbac.yaml
```
...
```bash
kubectl --kubeconfig="$HOME/labs/lab22/kubeconfig" --context=kind-lab22 --namespace=ce-lab22 \
  delete pod borrowed-pod --ignore-not-found
kubectl --kubeconfig="$HOME/labs/lab22/kubeconfig" --context=kind-lab22 --namespace=ce-lab22 \
  delete role within-rights beyond-rights --ignore-not-found
kubectl --kubeconfig="$HOME/labs/lab22/kubeconfig" --context=kind-lab22 --namespace=ce-lab22 \
  apply -f "$HOME/labs/lab22/rbac.yaml"
```
...
can-i get secrets should answer no for the builder account with a nonzero exit, and the Pod running as that account should report subject=system:serviceaccount:ce-lab22:builder and secrets -> 403, with a message naming the account, the verb and the resource. That is a genuine refusal, and it is the baseline the rest of the lab is compared against.
...
## Lab 23 — When probes cause or repair restarts
...
- Use ce-lab23 in its private cluster. One server initializes for 15 seconds; a separate client calls it through its Service. Two manifests differ only in the startup probe.
...
k apply -f ~/labs/lab23/no-startup.yaml
wait_restarts 1
...
```bash
k delete pod probe-web --wait=true --timeout=30s
k apply -f ~/labs/lab23/protected.yaml
k wait pod/probe-web --for=condition=Ready --timeout=90s --request-timeout=0
...
## Lab 24 — Volume binding, placement and retained data

**Question:** Why can a claim wait, a replacement Pod keep data, and a retained volume still refuse a new claim?

**Before you run:** Provision the shared VM once with ./lab.sh provision. Run ./lab.sh 24 setup on the host. No other lab is required.
...
WaitForFirstConsumer delays binding until the scheduler can consider a consuming Pod. A Pending claim without a consumer can therefore be normal. The local directory exists only on lab24-worker, and the PV declares that node affinity. A consumer requiring lab24-worker2 cannot satisfy both placement and storage topology, even when CPU, memory and storage capacity are sufficient.
...
- Use one 256 MiB declared local PV on lab24-worker, one 128 MiB claim and a sleeping consumer. The two consumer manifests differ only in nodeSelector. No external storage driver or download is needed.
...
k get storageclass ce-lab24-local -o yaml
storage_state baseline
k describe pv ce-lab24-data
```
...
```bash
k apply -f ~/labs/lab24/claim.yaml
...
```bash
k apply -f ~/labs/lab24/wrong-node.yaml
...
k apply -f ~/labs/lab24/consumer.yaml
...
k wait pv/ce-lab24-data --for=jsonpath='{.status.phase}'=Released --timeout=60s --request-timeout=0
storage_state released
k get pv ce-lab24-data -o yaml
docker exec lab24-worker cat /var/local/ce-lab24-data/lesson.txt
```
...
```bash
k delete pv ce-lab24-data --wait=true --timeout=30s
k apply -f ~/labs/lab24/volume.yaml -f ~/labs/lab24/claim.yaml -f ~/labs/lab24/consumer.yaml
k wait pod/store --for=condition=Ready --timeout=90s --request-timeout=0
k exec store -- cat /data/lesson.txt
...
**Recovery check:** The new claim is Bound, store is Ready on lab24-worker, and /data/lesson.txt still contains learned-persistence.
...
**Expected:** The static volume should be Available and tied to lab24-worker; the class delays binding.
...
## Lab 25 — Namespace budgets and admission failures
...
- Use ce-lab25 with sleeping Pods, a 200m CPU-request quota and explicit per-container defaults. The oversized manifest tests a memory-limit rule without consuming extra memory.
...
```bash
k create -f ~/labs/lab25/defaulted.yaml
echo "create exit=$?"
k get pod tenant-b
wait_budget 200m 100m
```
...
```bash
k apply -f ~/labs/lab25/defaults.yaml
k describe limitrange container-budget
k create -f ~/labs/lab25/oversized.yaml
echo "create exit=$?"
k get pod oversized
```
...
```bash
k create -f ~/labs/lab25/extra.yaml
echo "create exit=$?"
k get pod tenant-c
wait_budget 200m 200m
```
...
```bash
k apply -f ~/labs/lab25/quota.yaml
wait_budget 200m 200m
...
k apply -f ~/labs/lab25/baseline.yaml
...
## Lab 26 — Secret encryption and key-loss recovery
...
- Use two toy Secrets with password ce-lab26-demo. legacy exists before encryption; fresh is created afterward. Never substitute real credentials. Both providers and static Pod variants are prepared automatically in this private cluster.
...
```bash
k create secret generic fresh --from-literal=password=ce-lab26-demo
secret_value fresh
...
```bash
k annotate secret legacy ce-lab26/rewrite=encrypted --overwrite
secret_value legacy
...
**Recovery check:** The key1 API server is ready; legacy and fresh return ce-lab26-demo, and both stored records retain the lab-key1 encryption prefix.
...
Changing provider configuration affects subsequent writes. identity keeps old plaintext readable, but does not migrate it. Updating the annotation writes the existing object through the first provider; the password remains ce-lab26-demo while its stored prefix changes to the key1 encryption prefix.
